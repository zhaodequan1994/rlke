...
@implementation PasswordOtherView

-(UIView *)backView{
    
    if (!_backView) {
        _backView = [[UIView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH * 0.127, SCREEN_HEIGHT * 0.346, SCREEN_WIDTH * 0.746, 0.335*SCREEN_HEIGHT)];
...
-(UITextField *)accountTextfield{
    ...
    if (!_accountTextfield) {
        _accountTextfield = [[UITextField alloc] initWithFrame:CGRectMake(20, self.moneyLabel.frame.origin.y + self.moneyLabel.frame.size.height + 40, self.backView.frame.size.width - 40, 30)];
        _accountTextfield.placeholder = @"请输入支付账号";
...
-(UIView *)lineView{
    ...
    if (!_lineView) {
        _lineView = [[UIView alloc] initWithFrame:CGRectMake(20, self.accountTextfield.frame.origin.y + self.accountTextfield.frame.size.height + 5, self.backView.frame.size.width - 40, 1)];
...
-(UITextField *)passwordTextfield{
    ...
    if (!_passwordTextfield) {
        _passwordTextfield = [[UITextField alloc] initWithFrame:CGRectMake(20, self.lineView.frame.origin.y + self.lineView.frame.size.height + 20, self.backView.frame.size.width - 40, 30)];
        _passwordTextfield.placeholder = @"请输入密码 (支付密码即注册密码)";
...
-(UIView *)lineView1{
    ...
    if (!_lineView1) {
        _lineView1 = [[UIView alloc] initWithFrame:CGRectMake(20, self.passwordTextfield.frame.origin.y + self.passwordTextfield.frame.size.height + 5, self.backView.frame.size.width - 40, 1)];
        _lineView1.backgroundColor = RGB(230, 230, 230);
...
-(void)passWordOtherShow{
    ...
    [self.backView addSubview:self.accountTextfield];    [self.backView addSubview:self.lineView];
    [self.backView addSubview:self.passwordTextfield];    [self.backView addSubview:self.lineView1];
